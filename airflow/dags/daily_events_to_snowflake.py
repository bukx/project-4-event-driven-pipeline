...
def extract_events(**ctx):
    ...
    df = pg.get_pandas_df(
        "SELECT * FROM events WHERE timestamp >= %(s)s::timestamptz"
        " AND timestamp < %(e)s::timestamptz",
...
def load_to_snowflake(**ctx):
    ...
    try:
        ...
        fname = path.split("/")[-1]
        cursor.execute(
            f"COPY INTO analytics.events.raw_events FROM @events_stage/{fname}"
